...
export const Navbar = async ({ width }: Width) => {
  ...
  return (
    <div className="sticky top-0 z-50 border-b bg-white py-4 shadow-sm">
      <nav
        className={cn(
          "mx-auto flex max-w-[1400px] items-start justify-between gap-3 px-5 md:gap-0",
        ...
        <div className="flex">
          {user ? (
            <form action={continueListingWithId}>
              <button
                type="submit"
                className="mr-2 mt-1 line-clamp-1 hidden cursor-pointer text-nowrap rounded-full py-3 text-center text-[15px] font-semibold transition hover:bg-neutral-100 md:block md:px-4 lg:mr-4"
              >
                Airbnb your home
              </button>
            </form>
